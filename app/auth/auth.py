...
def get_userinfo(token: str = Depends(oauth2_scheme)) -> httpx.Response:
    headers = {
        'accept': 'application/json',
        'cache-control': 'no-cache',
        'content-type': 'application/x-www-form-urlencoded',
    ...
    url = settings.keycloak_url + '/userinfo'
...
has_access_to_admin_routes = RemoteAuthValidator(allowed_roles=settings.admin_roles)
has_access_to_prosumer_routes = RemoteAuthValidator(allowed_roles=settings.user_roles)
